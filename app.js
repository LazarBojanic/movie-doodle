...
/* --- Poster toggle --- */
posterBtn.addEventListener("click", ()=>{ showPoster=!showPoster; redraw(); });
...
async function fetchMovieById(id){
  ...
  try{
    ...
    if(!data || data.success===false){ movieTitleEl.textContent=""; posterImage=null; redraw(); return alert("Movie not found"); }
    movieTitleEl.textContent=`${data.title} (${(data.release_date||"").slice(0,4)})`;
    if(!data.poster_path){ posterImage=null; redraw(); return; }
    const img = new Image();
    ...
    img.onerror=()=>{ posterImage=null; redraw(); alert("Failed to load poster"); };
